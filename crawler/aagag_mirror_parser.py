...
class AagagMirrorParser:
    def get_article_list(self, soup_root):
        return seq(soup_root.find_all('table', class_='aList')).flat_map(lambda r: r.find_all('a', class_='article')).take(4)
...
if __name__ == '__main__':
    ...
    soup_root = BeautifulSoup(requests.get(url).text, 'html.parser')
    articles = parser.get_article_list(soup_root)
